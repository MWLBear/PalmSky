...
EVENT_TEMPLATES = [
    ...
    {   # Index 2: 赌 Buff (默认写的是自动，循环里会随机改成点击)
        "type": "buff_gamble",
        "desc_suffix": " 药力狂暴！",
...
#    if logic_type == "gain_auto_safe" or logic_type == "gain_tap_safe": return f"{text} (稳健)"
...
for stage_idx in range(16):
    ...
    print(f"   - 生成 [{current_stage_name}] 期... {count} 个")
    ...
    for _ in range(count):
        ...
        # 🔥🔥🔥 核心修复：在这里动态切换 Auto/Tap 赌局 🔥🔥🔥
        if template["type"] == "buff_gamble":
            # 50% 几率把 "赌自动" 换成 "赌点击"
            if random.random() < 0.5:
                logic_a = "gamble_buff_tap" # 赌点击
                logic_b = "gain_tap_safe"   # 对应的稳健选项也要换成点击
                suffix = " 指尖滚烫！"       # 文案微调
        ...
        event = {
            "id": f"evt_tap_fix_{global_id_counter:05d}",
            "title": title,
            "desc": full_desc,
...
print(f"\n✅ [完全体] 生成完毕！gamble_tap 和 gamble_auto 现在都会出现了。")
